...
1. 小红书发笔记/评论区引流到个人主页。
2. 收到订单 → 打开本地「码池 HTML」→ 点「一键取码发货」（自动取未售码、标记「已发出」、复制到剪贴板）→ 粘贴私信发给买家。
3. 买家到 Pleadly「设置 → 兑换码」填码 → 服务端校验、标记已用、积分到账。

> 小红书对个人号没有自动发码接口，用「半自动取码」即可；量大后再接第三方自动发货平台。
> 后端已配定时任务：每 30 分钟清理已兑换的码 + 未售码少于 5 个时自动补到 20 个（默认面额 60 积分）。
...
- **MVP 边界**：
  ...
  - `FREE_STARTER=0`（当前已不赠送积分），避免「换设备 ID」刷免费额度。
